...
void Enclosure::DisplayCellInfo() const
{
...
void Enclosure::PrintHistoryOfCell() const
{
    std::cout << "\nHistory of Cell " << m_uniqueCellId << "\n";
    for (auto& r : m_history)
    {
        std::cout << r.m_animalName << " (" << r.m_animalId << ") ";
        std::cout << " From " << r.m_entryDate << " To "
                   << (r.m_exitDate.empty() ? std::string("present") : r.m_exitDate) << "\n";
    }
}
